...
{
	// Ÿród³o: http://www.sandia.gov/~kmorel/documents/ColorMaps/
	// plik: CoolWarmUChar257.csv
	static const std::pair<double, Color> kolory_init[] =
	{
		{ 0, { 59, 76, 192 } },
	...
	static const std::vector<std::pair<double, Color>> kolory(std::begin(kolory_init), std::end(kolory_init));
	const double low = (x > srodek) ? srodek : min;
	const double high = (x > srodek) ? max : srodek;
	const double offset = (x > srodek) ? 0.5 : 0;
	const double y = offset + (x - low) / (high - low) * 0.5;
	assert(y >= 0 && y < 1);
	return std::lower_bound(kolory.begin(), kolory.end(), y, [](const std::pair<double, Color>& l, double r)
	{
		return l.first < r;
	})->second;
}